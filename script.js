...
function initializeLoader() {
  window.setTimeout(() => {
    ...
    window.setTimeout(() => { appLoader.remove(); }, 800);
  }, 900);
}
...
function createImagePreview(file, previewBox, previewImage, fileNameElement) {
  ...
  if (!validation.valid) {
    showToast("Invalid Image", validation.message, "error");
    return false;
  }
...
function initializeRealAnalysis() {
  ...
  input.addEventListener("change", () => {
    ...
    if (!file) return;
    const success = createImagePreview(file, previewBox, previewImage, fileName);
    if (!success) {
      input.value = "";
      return;
    }
    showToast("Chart Ready", "Starting AI chart analysis...", "success");
    startAnalysis({
      type: "real",
      input,
...
function initializeOtcAnalysis() {
  ...
  input.addEventListener("change", () => {
    ...
    showToast("OTC Chart Ready", "Starting OTC analysis...", "success");
    startAnalysis({
      type: "otc",
      input,
...
function startAnalysis(options) {
  ...
  const progressTimer = window.setInterval(() => {
    ...
    const percent = Math.min(100, Math.round((elapsed / APP.analysisDuration) * 100));
    ...
    if (newStep !== currentStep && newStep >= 0) {
      if (currentStep >= 0) {
        ...
        const previousIcon = steps[currentStep].querySelector("span");
        if (previousIcon) previousIcon.textContent = "✓";
      }
      currentStep = newStep;
      steps[currentStep].classList.add("active");
      const activeIcon = steps[currentStep].querySelector("span");
...
function finishAnalysis(options, startTime) {
  ...
  window.setTimeout(() => {
    ...
    applySignal(signalElement, signalText, result.direction);
    ...
    if (pattern) pattern.textContent = result.pattern;
    if (momentum) momentum.textContent = result.momentum;

    const duration = ((performance.now() - startTime) / 1000).toFixed(1);
    if (resultTime) resultTime.textContent = `${duration}s`;
    ...
    showToast("Analysis Complete", `${result.direction} signal generated.`, "success");
  }, 350);
}
...
function generateDemoAnalysis() {
  ...
  return {
    ...
    momentum: momentumList[Math.floor(Math.random() * momentumList.length)]
  };
}
...
function initializeFutureSignals() {
  ...
  generateButton.addEventListener("click", () => {
    ...
    if (!market) {
      showToast("Select Market", "Please select a market first.", "error");
      marketSelect.focus();
      return;
    }
    ...
    window.setTimeout(() => {
      const signals = generateFutureSignals(market, 10);

      if (signalList) signalList.innerHTML = "";
      if (selectedMarketName) selectedMarketName.textContent = `${market} Signals`;

      signals.forEach((signal, index) => {
        createSignalCard(signal, index, signalList);
      });
      ...
      showToast("Signals Ready", "10 signal ideas generated.", "success");
    }, 1800);
  });
}
...
function generateFutureSignals(market, amount) {
  ...
  for (let i = 0; i < amount; i++) {
    ...
    const confidence = Math.floor(70 + Math.random() * 27);
    const signalTime = new Date(now.getTime() + ((i + 1) * 2 * 60 * 1000));
...
function createSignalCard(signal, index, container) {
  ...
  card.innerHTML = `
    <div class="signal-number">${number}</div>
    <div class="future-signal-market">
      <strong>${escapeHTML(signal.market)}</strong>
...
function showToast(title, message, type = "success") {
  ...
  if (icon) {
    if (type === "error") {
      ...
      icon.style.background = "rgba(255,64,92,0.08)";
    } else {
      icon.textContent = "✓";
      icon.style.color = "var(--green)";
      icon.style.background = "rgba(0,230,118,0.08)";
    }
  }
...
document.addEventListener("visibilitychange", () => {
  if (document.hidden) {
    if (engineStatus) engineStatus.textContent = "Paused";
  } else {
    if (engineStatus) engineStatus.textContent = "Ready";
  }
});
...
console.log("%c Ai Analyses ", "background:#071321;color:#00e5ff;font-weight:bold;padding:8px;");
